const contractAddress = "0xaFF4481D10270F50f203E0763e2597776068CBc5";
...
async function main() {
    ...
    let instance = await new ethers.Contract(contractAddress, weenus_abi, provider1);
    console.log(instance.functions);
    let tx = await instance.drip();
    // let tx = await instance.connect(provider2).changeMyName("Second Change", {gasPrice: price});
    // let tx = await instance.connect(provider1).createPerson("HELLOOOO", {gasPrice: price});
    // tx.wait();
    // console.log(tx);
    // console.log("done");
}
